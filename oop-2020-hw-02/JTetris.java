...
public class JTetris extends JComponent {
	...
	JTetris(int pixels) {
		super();
		if (pixels <= 0)
			throw new RuntimeException("pixels should be positive number.");
	...
	*/
	public void computeNewPosition(int verb) {
		...
		switch (verb) {
			...
			case ROTATE:
				...
				if(newX+newPiece.getWidth() > board.getWidth()) {
					newX = board.getWidth() - newPiece.getWidth() ;
				} else if(newX < 0){
					newX++;
				}
				break;
	...
	protected void tickHelper(int verb){
	...
	*/
	...
	public void controlPanelHelper(JPanel panel){
