...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		int bricks = points.length;
		body = new TPoint[bricks];
		width = 0;
		height = 0;
		// I choose Treemap because it sorts by key
		Map<Integer, Integer> skirtMap = new TreeMap<Integer, Integer>();
		for(int i=0; i<bricks; i++)
			addBrick(i, skirtMap, points);
		Arrays.sort(body);
		skirt = new int[skirtMap.size()];
		for(int i=0; i<skirtMap.size(); i++) skirt[i] = skirtMap.get(i);
	}

	/** Helper function for constructor: adds one point to the Piece.
	  */
	private void addBrick(int i, Map<Integer, Integer> skirtMap, TPoint[] points){
		body[i] = new TPoint(points[i]);
		if(width<(body[i].x+1)) width = body[i].x + 1;
		if(height<(body[i].y+1)) height = body[i].y + 1;
		if(skirtMap.containsKey(body[i].x)){
			if(skirtMap.get(body[i].x) > body[i].y) skirtMap.put(body[i].x,body[i].y);
		} else {
	...
	*/
	...
	/**
	   Converts body of a piece to a boolean 2d grid.
	 */
	public boolean[][] toBoard(){
		boolean[][] result = new boolean[width][height];
		for(int i=0; i<body.length; i++){
			result[body[i].x][body[i].y] = true;
		}
		return result;
	}
	...
	public Piece computeNextRotation() {
		...
		int minx = 0;
		for(int i=0; i<body.length; i++){
			newBody[i] = new TPoint(-1*body[i].y, body[i].x);
			if(minx > newBody[i].x) minx = newBody[i].x;
		}
		if(minx<0) {
			for (int i=0; i<newBody.length; i++)
				if(minx<0) newBody[i].x -= minx;
		}
	...
	*/
	public boolean equals(Object obj) {
		...
		//Remember the body array is already sorted in ctor:
